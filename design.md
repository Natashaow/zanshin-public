...
Signature interactions `[LOCKED — built in the 2026-08-02 Dashboard *artifact* (Pass 02); NOT implemented in this app as of 2026-08-15]`. Natasha's directive (2026-08-15, relayed via the strategy session): combine the old animated Zanshin design into this build — the port is the next code task for this phase. `framer-motion@12` is already installed and currently has zero uses; port with it, pulling durations/easings from the §3.2 tokens (shipped). Note the Void ground was chosen *because* it serves the Euthymia mode — landing it retroactively justifies the palette decision. **Pre-port fix — done 2026-08-16, no longer a blocker**: `reorder()` in `App.jsx` had an off-by-one on downward drags (`toIndex` computed pre-removal, so the item landed one slot past the target while upward drags were correct). It is fixed at `App.jsx:270`, which compensates with `fromIndex < toIndex ? toIndex - 1 : toIndex`. Animating the drop no longer polishes a wrong landing slot.
