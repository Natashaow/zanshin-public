...
- **The `ask` tier cannot be self-verified, and that is a permanent property, not a to-do.** A session **cannot observe its own permission prompts** — it sees a permitted call and a completed result whether or not a human approved it. No test a session runs on itself closes this. The tier *is* verified here: the operator confirmed prompts fired, which also demonstrated an `ask` beating a matching `allow` in a lower-precedence settings file, as the documented evaluation order predicts. But that confirmation came from the person watching the screen, and any future claim about prompting behaviour needs the same. This paragraph read "configured but unverified" until someone actually checked.
