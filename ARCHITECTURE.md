...
- ~~**2026-08-15** — `ACTIVE_PROJECTS` carries no real `lifeBucket`/`dueDate` data yet, so `deriveLifeBuckets()` puts everything in "Uncategorized" and the Active Arcs table's lifeBucket/dueDate cells render empty.~~ Closed 2026-08-19 for `lifeBucket` — `sync-vault-data.mjs` reads each project hub's `bucket/<name>` frontmatter tag (real data, 7 of 8 active projects tagged). `dueDate` stays permanently absent, not a gap — the vault has no per-project due-date field to read, so it's left unset rather than fabricated. `src/data/generated/activeProjects.json` (the public/fresh-clone fixture) now carries illustrative `lifeBucket` values too, so the feature is visible without local vault access.
